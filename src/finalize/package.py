...
def main():
 ...
 clean.groupby('dataset')[clean.select_dtypes('number').columns].agg(['mean','std']).to_csv(final/'table_clean_baselines.csv')
 adv.groupby(['dataset','attack','budget'])[adv.select_dtypes('number').columns].agg(['mean','std']).to_csv(final/'table_adversarial_results.csv')
